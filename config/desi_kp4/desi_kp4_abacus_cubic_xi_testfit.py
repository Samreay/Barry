...
if __name__ == "__main__":
    ...
    sampler = DynestySampler(temp_dir=dir_name, nlive=250, print_progress=True)
    ...
    name = dataset.name + f" fixed sigmas " + " mock mean"
    ...
    if fitter.should_plot():
        ...
        for posterior, weight, chain, evidence, model, data, extra in fitter.load():
            ...
            print(
                params_dict,
                model.get_alphas(params_dict["alpha"], params_dict["epsilon"]),
                bband,
                -0.5 * posterior[max_post],
                new_chi_squared,
                dof,
            )
            ...
            print(mean, np.sqrt(cov))

        truth = {"$\\Omega_m$": 0.3121, "$\\alpha$": 1.0, "$\\epsilon$": 0, "$\\alpha_\\perp$": 1.0, "$\\alpha_\\parallel$": 1.0}
        c.configure(bins=20)
        c.plotter.plot(truth=truth, parameters=["$\\alpha_\\parallel$", "$\\alpha_\\perp$"], legend=False, display=True)
