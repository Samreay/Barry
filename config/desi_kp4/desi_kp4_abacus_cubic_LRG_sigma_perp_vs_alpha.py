    ...
    if fitter.should_plot():
        ...
        for posterior, weight, chain, evidence, model, data, extra in fitter.load():
            ...
            stats[data_bin].append(
                [sigma_nl_perp[sigma_bin], len(model.n_poly), mean[0] - 1.0, mean[1] - 1.0, np.sqrt(cov[0, 0]), np.sqrt(cov[1, 1])]
            )
            output[datanames[data_bin]].append(
                f"{sigma_nl_perp[sigma_bin]:6.4f}, {len(model.n_poly):3d}, {mean[0]-1.0:6.4f}, {mean[1]-1.0 :6.4f}, {np.sqrt(cov[0, 0]):6.4f}, {np.sqrt(cov[ 1, 1]):6.4f}"
            )
