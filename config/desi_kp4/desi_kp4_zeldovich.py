    ...
    for i, (mocktype, redshift_bins) in enumerate(zip(mocktypes, nzbins)):
        ...
        for z in range(redshift_bins):
            ...
            for recon in ["sym"]:
                ...
                # Overwrite the mock data with Stephen's theoretical curves
                pkdata = np.loadtxt("../../barry/data/desi_kp4/pkells_recsym_theory.txt")
                dataset.ks = pkdata[dataset.w_mask, 0]
                dataset.data = np.vstack(
                    (
                        pkdata[dataset.w_mask, 1],
                        np.zeros(len(dataset.ks)),
                        pkdata[dataset.w_mask, 2],
                        np.zeros(len(dataset.ks)),
                        np.zeros(len(dataset.ks)),
                    ),
                ).T
                ...
                model.set_default("sigma_nl_par", 6.0)
                ...
                xidata = np.loadtxt("../../barry/data/desi_kp4/xiells_recsym_theory.txt")
                dataset.ss = xidata[dataset.mask, 0]
                dataset.data = np.vstack(
                    (
                        xidata[dataset.mask, 1],
                        xidata[dataset.mask, 2],
                        np.zeros(len(dataset.ss)),
                    ),
                ).T
