...
def plot_alphas(stats, figname, type="xi"):
    ...
    for n_poly in range(colmax - colmin):
        ...
        axes[0, n_poly].set_xlim(0.0, 4.4)
        axes[1, n_poly].set_xlim(0.0, 4.4)
        axes[0, n_poly].set_ylim(-0.75, 0.75)
        axes[1, n_poly].set_ylim(-0.45, 0.45)
        axes[1, n_poly].set_xlabel(r"$\Sigma_{s}$")
        if n_poly == 0:
...
if __name__ == "__main__":
    ...
    if fitter.should_plot():
        ...
        for posterior, weight, chain, evidence, model, data, extra in fitter.load():
            ...
            stats[data_bin].append([sigma_s[sigma_bin], model.n_poly, mean[0] - 1.0, mean[1] - 1.0, np.sqrt(cov[0, 0]), np.sqrt(cov[1, 1])])
            output[datanames[data_bin]].append(
                f"{sigma_s[sigma_bin]:6.4f}, {model.n_poly:3d}, {mean[0]-1.0:6.4f}, {mean[1]-1.0:6.4f}, {np.sqrt(cov[0, 0]):6.4f}, {np.sqrt(cov[1, 1]):6.4f}"
            )
