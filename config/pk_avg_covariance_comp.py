...
if __name__ == "__main__":
    ...
    for r in [True, False]:
        ...
        fitter.add_model_and_dataset(PowerSeo2016(recon=r), d, name=f"Seo 2016 {t}", linestyle=ls, color=cs[2])
        fitter.add_model_and_dataset(PowerDing2018(recon=r), d, name=f"Ding 2018 {t}", linestyle=ls, color=cs[3])
        fitter.add_model_and_dataset(PowerNoda2019(recon=r, postprocess=p), de, name=f"Noda 2019 {t}", linestyle=ls, color=cs[4])
    ...
    if fitter.should_plot():
        ...
        c.plotter.plot_summary(filename=[pfn + "_summary2.png", pfn + "_summary2.pdf"], extra_parameter_spacing=2.5, parameters=1, errorbar=True, truth={"$\\Omega_m$": 0.31, '$\\alpha$': 1.0})
