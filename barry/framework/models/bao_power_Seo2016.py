...
class PowerSeo2016(PowerSpectrumFit):
    ...
    def __init__(self, fit_omega_m=False, fit_growth=False, smooth_type="hinton2017", recon=False, recon_smoothing_scale=10.0, name="BAO Power Spectrum Seo 2016 Fit"):
        self.recon = recon
        ...
        if not self.fit_omega_m:
            self.sigma, self.sigma_dd, self.sigma_ss, _, _, _, _, _, _, self.R1, self.R2 = self.PT.get_data(om=self.omega_m)
    ...
    def compute_power_spectrum(self, k, p):
        ...
        if self.fit_omega_m:
            sigma, sigma_dd, sigma_ss, _, _, _, _, _, _, R1, R2 = self.PT.get_data(om=p["om"])
        else:
        ...
        if self.recon:
            ...
            damping_dd = np.exp(-np.outer(1.0 + (2.0 + growth)*growth*mu**2, ks**2)*sigma_dd/2.0)
            damping_ss = np.exp(-np.tile(ks**2, (nmu, 1))*sigma_ss/2.0)
            kaiser = 1.0 + np.outer(growth/p["b"]*mu**2, 1.0-smoothing_kernel)
            smooth_prefac = np.tile(smoothing_kernel/p["b"], (nmu, 1))
            propagator = (kaiser*damping_dd + smooth_prefac*(damping_ss-damping_dd))**2
        else:
            damping = np.exp(-np.outer(1.0 + (2.0 + growth)*growth*mu**2, ks**2)*sigma/2.0)
            prefac_k = 1.0 + np.tile(3.0/7.0*(R1*(1.0-4.0/(9.0*p["b"]))+R2), (nmu, 1))
            prefac_mu = np.outer(mu**2, growth/p["b"] + 3.0/7.0*growth*R1*(2.0-1.0/(3.0*p["b"])) + 6.0/7.0*growth*R2)
...
if __name__ == "__main__":
    ...
    p = {"om": 0.3, "alpha": 1.0, "sigma_s":10.0, "b": 1, "a1": 0, "a2": 0, "a3": 0, "a4": 0, "a5": 0}
