...
if __name__ == "__main__":
    ...
    for gc in ["NGC"]:  # , "SGC"]: # Dont have window functions for SGC
        ds = [f"post_recon {gc}/", f"pre_recon {gc}/"]
        files = [d + f for d in ds for f in os.listdir(d)]
        print(files)
        files.sort(key=lambda s: int(s.split("_z3_")[1].split("_")[0]))
        print([f.split("_z3_")[1].split("_")[0] for f in files if "post_recon" in f])
        ...
        split = {
            ...
            "name": f"SDSS DR12 Z0.61 Pk {gc}",
            "winfit": get_winfits("./"),
            "winpk": get_winpk("./"),
        }

        with open(f"../sdss_dr12_z061_pk_{gc.lower()}.pkl", "wb") as f:
            pickle.dump(split, f)
