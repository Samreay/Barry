...
if __name__ == "__main__":
    ...
    with open(f"../desi_mock_challenge_post_stage_2_pk_pre_std.pkl", "wb") as f:
        pickle.dump(split, f)
    ...
    files = [
        [
            ...
            ds + "UNIT_DESI_Shadab_HOD_snap97_ELG_v1_4col.gcat-smooth5-bias1.415-niter4.RSD_powspec.dat",
        ],
        [
            ds + "UNIELG-b0s10rsd0g1536postmultipoles.txt",
            ds + "UNIT_DESI_Shadab_HOD_snap97_ELG_v1_4col.gcat-smooth10-bias1.415-niter4.RSD_powspec.dat",
        ],
        [
            ds + "UNIELG-b0s15rsd0g1536postmultipoles.txt",
            ds + "UNIT_DESI_Shadab_HOD_snap97_ELG_v1_4col.gcat-smooth15-bias1.415-niter4.RSD_powspec.dat",
            ds + "hj_reciso1024_rsd_reconstructed_multipoles.txt",
        ],
        [
            ds + "UNIELG-b0s20rsd0g1536postmultipoles.txt",
            ds + "UNIT_DESI_Shadab_HOD_snap97_ELG_v1_4col.gcat-smooth20-bias1.415-niter4.RSD_powspec.dat",
        ],
    ]
    for i, smooth in enumerate([5, 10, 15, 20]):
        ...
        cov_filename = ds + f"/cov_matrix_pk-EZmocks-3Gpc_rsd_centerbin_RecIso_post.txt"
        ...
        split = {
            ...
            "name": f"DESI Mock Challenge Stage 2 RecIso" + str(smooth),
            "winfit": getwin(ks),
            "winpk": None,  # We can set this to None; Barry will set it to zeroes given the length of the data vector.
            "m_mat": getcomp(ks),
        }
    ...
    files = [
        [ds + "UNIELG-b0s05rsd0g1536postmultipoles.txt"],
        [ds + "UNIELG-b0s10rsd0g1536postmultipoles.txt"],
        [ds + "UNIELG-b0s15rsd0g1536postmultipoles.txt", ds + "hj_recani1024_reconstructed_multipoles.txt"],
        [ds + "UNIELG-b0s20rsd0g1536postmultipoles.txt"],
    ]
    ...
    for i, smooth in enumerate([5, 10, 15, 20]):
        ...
        cov_filename = ds + f"/cov_matrix_pk-EZmocks-3Gpc_rsd_centerbin_post.txt"
        res = {f.lower(): getpk_post(f) for f in files[i]}
        ks = next(iter(res.items()))[1]["k"].to_numpy()
        cov = pd.read_csv(cov_filename, delim_whitespace=True, header=None).to_numpy()
        cov_flat = cov.astype(np.float64)[:, 2]
        ...
        split = {
            ...
            "name": f"DESI Mock Challenge Stage 2 RecAni" + str(smooth),
            "winfit": getwin(ks),
            "winpk": None,  # We can set this to None; Barry will set it to zeroes given the length of the data vector.
            "m_mat": getcomp(ks),
        }
